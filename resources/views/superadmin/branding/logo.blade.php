            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 space-y-6">
                    <div>
                        ...
                        @if($siteLogoPath)
                            <img src="{{ asset('storage/' . $siteLogoPath) }}" class="h-16 w-auto" alt="{{ config('app.name') }}" />
                        @else
                            <x-application-logo class="h-16 w-auto fill-current text-gray-800" />
                        @endif
                    </div>

                    <form action="{{ route('superadmin.branding.logo.update') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                        @csrf
