                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                       <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Member</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse ($deposits as $deposit)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">{{ $deposit->member->name ?? 'N/A' }}</td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <a href="{{ route('deposits.show', $deposit) }}" class="text-gray-600 hover:text-gray-900">View</a>
                                        @can('deposits.manage')
                                        <a href="{{ route('deposits.edit', $deposit) }}" class="text-indigo-600 hover:text-indigo-900 ml-2">Edit</a>
                                        <form action="{{ route('deposits.destroy', $deposit) }}" method="POST" class="inline-block ml-2" onsubmit="return confirm('Are you sure?');">
                                            @csrf
                                            @method('DELETE')
